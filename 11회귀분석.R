...
# 예상제동거리, 실제제동거리 ,오차구하기 를 데이터 프레임으로 한번에 만들기
...
model.2
# 결과 회귀식 : y= 2.591 x + (-1.312)
w <- coef(model.2)[2]
b <- coef(model.2)[1]

#3단계 age가 10, 15, 20일 때 height 를 예측하기
age <- 10
y.10 <- w* age + b
y.10
...
# 3. pressure 데이터셋에서 온도(temperature)로 기압(pressure)을 예측하는 단순선형 회귀모델을 만드시오. 
# 그리고 온도가 65, 95, 155일 때 기압을 예측하여 보시오.
head (pressure)
dim(pressure)
str(pressure)
plot(pressure~temperature, data=pressure, col='green')
tmod <- lm(pressure~temperature, pressure)
tmod
abline(tmod)
w <- coef(tmod)[2]
b <- coef(tmod)[1]
