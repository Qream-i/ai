...
-- 1. json 만들기
-- json.array()
select json_array(1, "abc", Null, True, curtime() ); -- [] 베열, 리스트
-- json_object()
...
select * from usertbl;
-- 키 userid , addr 값 json 을 만들기
select  
	json_object( userid, addr ) 
from usertbl;

-- 키 userid, 값 [010,0000000] 형태로 json 마들기
select
	json_object( userid, json_array(mobile1, mobile2))
from usertbl;

-- 외부로 내부내기 > json 파일로 만들기
select
	json_object( userid, json_array(mobile1, mobile2))
from usertbl
...
insert into stdclubtbl values (null, '김범수','바둑'),(null,'김범수','축구'), (null,'조용필','축구'),(null,'은지원','축구'),(null,'은지원','축구'),(null,'은지원','봉사'),(null,'바비킴','봉사');