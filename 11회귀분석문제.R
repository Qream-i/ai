...
summary(model.4)
# 독립변수 중 기여도가 가장 높은 변수는 ?
# *갯수에 따라, 즉 HS.Grad의 기여도가 가장 높다.
# 유의 수준은 p-value, 즉 1.057e-05 이다.
# 모델의 설명력 = Adjusted R-squared으로, 0.3597이다. 
# (2) 다중선형 회귀모델을 이용하여 state.x77 데이터셋의 문맹률(Illiteracy)과 인구수(Population)로 1인당 소득(Income)을 예측하시오.
model.5 <- lm(Income~Illiteracy + Population, data= data.frame(state.x77))
model.5
# 회귀식 ㅛ= -468.63466 * Illiteracy + 0.03555 * Population = 4833.16224
summary (model.5)
...
# 종속변수(예측하려는 값) : 주택 가격(medv), 독립변수(사용/이용되는 값) : (medv, chas를 제외한) 나머지 변수
# 회귀식 구하기 4번쨰에 위치한 medv
...
summary(model.6)
# y =-0.11144* cyl + 0.01334 * disp + (-0.02148) * hp + ... + (-0.19942) * carb + 12.30337
...
summary(model.6)
summary(model.6.1)
# 결과 분석 보고서 : 독립변수를 모두 이용한 model.6의 설명력은 0.8066이고,
#                    불필요한 독립변수를 제거한 모델 model.6.1의 설명력은 0.8336이므로 model.6.1을 선택한다.
